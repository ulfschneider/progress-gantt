...
},{}],"progress-gantt":[function(require,module,exports){
"use babel";
...
function lineHeight(settings) {
  return Math.min(
    settings.y.bandwidth() / (2 + labelLineFactor(settings)),
...
function validateSettings(settings) {
  ...
  settings.showTimeAxis = _.isUndefined(settings.showTimeAxis)
    ? true
    : settings.showTimeAxis;
  ...
  if (_.isUndefined(settings.margin) || _.isEmpty(settings.margin)) {
    ...
    settings.margin.left = _.isUndefined(settings.margin.left)
      ? 0
      : settings.margin.left;
    settings.margin.top = _.isUndefined(settings.margin.top)
      ? 50
      : settings.margin.top;
    settings.margin.right = _.isUndefined(settings.margin.right)
      ? 50
      : settings.margin.right;
    settings.margin.bottom = _.isUndefined(settings.margin.bottom)
      ? 50
  ...
  if (_.isUndefined(settings.style) || _.isEmpty(settings.style)) {
    ...
    settings.style.backgroundColor = settings.style.backgroundColor
      ? settings.style.backgroundColor
      : "#fff";
    settings.style.color = settings.style.color ? settings.style.color : "#222";
    settings.style.todayColor = settings.style.todayColor
      ? settings.style.todayColor
      : settings.style.color;
    settings.style.labelColor = settings.style.labelColor
      ? settings.style.labelColor
      : settings.style.color;
    settings.style.barColor = settings.style.barColor
      ? settings.style.barColor
      : "#ccc";
    settings.style.progressBarColor = settings.style.progressBarColor
      ? settings.style.progressBarColor
      : settings.style.color;
    settings.style.overrunBarColor = settings.style.overrunBarColor
      ? settings.style.overrunBarColor
      : settings.style.barColor;
    settings.style.overrunProgressBarColor = settings.style
      .overrunProgressBarColor
      ? settings.style.overrunProgressBarColor
      : settings.style.progressBarColor;
    settings.style.fontFamily = settings.style.fontFamily
      ? settings.style.fontFamily
      : "sans-serif";
    settings.style.fontSize = settings.style.fontSize
      ? settings.style.fontSize
      : 12;
    ...
    if (!settings.style.axis) {
      ...
      settings.style.axis.color = settings.style.axis.color
        ? settings.style.axis.color
        : settings.style.color;
    }
  }
...
function prepareDataFunctions(settings) {
  ...
  settings.fromDate = d3.min(settings.data, function (d) {
    if (d.startDate) {
  ...
  settings.toDate = d3.max(settings.data, function (d) {
...
function drawAxis(settings) {
  if (
    equalsIgnoreCase(settings.showTimeAxis, "top") ||
    (settings.showTimeAxis &&
      !equalsIgnoreCase(settings.showTimeAxis, "bottom") &&
      !equalsIgnoreCase(settings.showTimeAxis, "false"))
  ) {
    settings.topAxis = settings.g.append("g").call(
      d3
        .axisTop(settings.x)
        .ticks(Math.floor(settings.innerWidth / 120))
  ...
  if (
    ...
    settings.bottomAxis = settings.g
      .append("g")
      .attr("transform", "translate(0," + settings.innerHeight + ")")
      .call(
        d3
          .axisBottom(settings.x)
          .ticks(Math.floor(settings.innerWidth / 120))
          .tickFormat(d3.timeFormat("%b %d"))
          .tickSize(TICK_SIZE)
      );
...
function drawText({ text, textAnchor, x, y, color, settings }) {
  settings.g
    .append("text")
    .attr("x", x)
    .attr("y", y)
...
function isInTimeRange(d, settings) {
  ...
  let end = getStartOfDay(
    d.overrun && !d.overrunDate
      ? getStartOfDay(getMoment())
      : d.overrunDate || d.endDate
  );
  return (
    start &&
    end &&
    settings.fromDate.valueOf() <= start.valueOf() &&
...
function drawTimeConsumptionBars(settings) {
  settings.g
    .selectAll(".bar")
    .data(
      settings.data.filter(function (d) {
    ...
    .attr("class", "bar")
    .attr("x", function (d) {
      return settings.x(getStartOfDay(d.startDate || settings.fromDate));
    })
    .attr("width", function (d) {
      if (d.overrun && !d.overrunDate) {
        return (
          settings.x(getStartOfDay(getMoment())) -
          settings.x(getStartOfDay(d.startDate || settings.fromDate))
        );
      } else if (d.overrunDate) {
        return (
          settings.x(getStartOfDay(d.overrunDate)) -
          settings.x(getStartOfDay(d.startDate || settings.fromDate))
        );
      } else {
        return (
          settings.x(getStartOfDay(d.endDate)) -
          settings.x(getStartOfDay(d.startDate || settings.fromDate))
        );
      }
    })
    .attr("y", function (d) {
      return settings.y(d.label);
    })
...
function drawOverrunBars(settings) {
  settings.g
    ...
    .data(
      settings.data.filter(function (d) {
        return (
          isInTimeRange(d, settings) &&
          d.endDate &&
          (d.overrun || d.overrunDate) &&
          getStartOfDay(d.overrunDate).valueOf() >
            getStartOfDay(d.endDate).valueOf()
        );
      })
    )
    .enter()
    ...
    .attr("x", function (d) {
      return settings.x(getStartOfDay(d.endDate));
    })
    .attr("width", function (d) {
      if (d.overrunDate) {
        return (
    ...
    .attr("y", function (d) {
      return (
        settings.y(d.label) +
        settings.y.bandwidth() -
        lineHeight(settings) -
        overrunBarHeight(settings)
      );
    })
...
function drawProgressBars(settings) {
  settings.g
    ...
    .attr("y", function (d) {
      return (
        settings.y(d.label) + settings.y.bandwidth() - lineHeight(settings)
      );
    })
    .attr("height", lineHeight(settings))
    .attr("fill", function (d) {
      return d.overrun || d.overrunDate
        ? settings.style.overrunProgressBarColor
        : settings.style.progressBarColor;
    })
    .on("click", function (d) {
...
function drawBarLabels(settings) {
  const addBarLabels = function (d, i) {
    d3.select(this)
      ...
      .attr("y", function (d) {
        return settings.y(d.label) + lineHeight(settings) / 2;
      })
    ...
    d3.select(this)
      ...
      .attr("y", function (d) {
        return (
          settings.y(d.label) + lineHeight(settings) + lineHeight(settings) / 2
        );
      })
      .attr("dominant-baseline", "central")
      .attr("font-size", fontSize(settings) + "px")
  ...
  settings.g
    ...
    .data(
      settings.data.filter(function (d) {
        return d.startDate || settings.fromDate;
      })
    )
    .enter()
    .append("g")
...
function drawDateLabels(settings) {
  ...
  settings.g
    ...
    .style("fill", settings.style.labelColor)
    .text(function (d) {
      return d.startDate ? formatTime(getMoment(d.startDate)) : "";
    })
    .on("click", function (d) {
      if (d.onClick) {
        d.onClick(d, d3.event);
...
function drawProgressLabels(settings) {
  ...
  const addProgressLabels = function (d, i) {
    let progressLabel = d3
      ...
      .text(function (d) {
        return formatPercentage(d.progress);
      });
    ...
    if (d.progressTag) {
      ...
      let text = d3
        ...
        .style(
          "fill",
          d.progressTagTextColor
            ? d.progressTagTextColor
            : settings.style.backgroundColor
        )
        .text(function (d) {
          return d.progressTag;
        });
      ...
      rect
        ...
        .attr("width", function (d) {
          return getBackgroundWidth(text);
        })
        .attr("height", function (d) {
          return fontSize(settings);
        });
    }
  };
...
function isDateInRange(date, settings) {
  ...
  if (
    settings.fromDate.valueOf() <= momentDate.valueOf() &&
    momentDate.valueOf() <= settings.toDate.valueOf()
  ) {
    return true;
  }
  return false;
...
function drawProgressGantt(settings) {
  ...
  d3svg.attr("width", settings.width).attr("height", settings.height);
...
},{"d3":32,"js-base64":33,"moment":34,"underscore":35}]},{},[]);
